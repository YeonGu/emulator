...
#define NEW_MAP( idx_, name_, type_, addr_, size_, map_ ) \
    ppu_memory_map[ idx_ ].map_name  = name_;             \
    ppu_memory_map[ idx_ ].nes_begin = addr_;             \
    ppu_memory_map[ idx_ ].map_size  = size_;             \
    ppu_memory_map[ idx_ ].map_begin = map_;              \
    ppu_memory_map[ idx_ ].map_type  = type_
...
void init_ppu_memory_map()
{
    ...
    ppu_memory_map[ 6 ].mirror_to = 0x2000;
    ...
    ppu_memory_map[ 8 ].mirror_to = 0x3F00;
    nr_ppu_map                    = 9;
}
